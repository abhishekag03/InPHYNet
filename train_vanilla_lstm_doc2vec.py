...
if not os.path.exists('checkpoints_vanilla_lstm_doc2vec'):
		os.makedirs('checkpoints_vanilla_lstm_doc2vec')
...
primary_task_network=PrimaryTaskNetwork(FLAGS.batch_size, FLAGS.prim_num_multi_classes, FLAGS.num_hidden, len(vocab), FLAGS.embedding_length, word_embeddings)
	...
	for idx, batch in enumerate(train_iterator):
		...
		if(idx%20==0):
			...
			with torch.no_grad():			
				...
				print_evaluation_metrics(np_targets, np_preds, 'PTN', os.path.join('checkpoints_vanilla_lstm_doc2vec', FLAGS.log_file))
			
	print('-------------------------------------------------------')
	if(epoch%5==0):
		state={'epoch': epoch, 'state_dict': primary_task_network.state_dict(), 'optimizer': optim.state_dict()}
		torch.save(state, os.path.join('checkpoints_vanilla_lstm_doc2vec', FLAGS.primary_task_network_save))
		pickle.dump([primary_task_network.word_embeddings.weight, primary_task_network.word_embeddings.weight.shape], open(os.path.join('checkpoints_vanilla_lstm_doc2vec', FLAGS.prim_vocab_weights_save), 'wb'))
