...
if not os.path.exists('checkpoints_inphynet_doc2vec'):
		os.makedirs('checkpoints_inphynet_doc2vec')
...
primary_task_network=PrimaryTaskNetwork(FLAGS.batch_size, FLAGS.prim_num_multi_classes, FLAGS.num_hidden, len(prim_vocab), FLAGS.embedding_length, prim_word_embeddings)
...
if FLAGS.load_saved:
	primary_state=torch.load(os.path.join('checkpoints_inphynet_doc2vec', FLAGS.primary_task_network_save))
	...
	aux_state=torch.load(os.path.join('checkpoints_inphynet_doc2vec', FLAGS.aux_task_network_save))
	aux_task_network.load_state_dict(aux_state['state_dict'])
	atn_optimizer.load_state_dict(aux_state['optimizer'])

	aux_t_state=torch.load(os.path.join('checkpoints_inphynet_doc2vec', FLAGS.aux_transfer_layer_save))
	aux_transfer_network.load_state_dict(aux_t_state['state_dict'])
	atl_optimizer.load_state_dict(aux_t_state['optimizer'])

	wal_state=torch.load(os.path.join('checkpoints_inphynet_doc2vec', FLAGS.weight_alignment_layer_save))
...
for epoch in range(start_epoch, end_epoch+1):
	...
	for idx, (prim_batch, aux_batch) in enumerate(zip(prim_train_iterator, aux_train_iterator)):
		...
		nn.utils.clip_grad_norm(ptn_params, max_norm=20)
		ptn_optimizer.step()
		...
		# 		print_evaluation_metrics(prim_np_targets, prim_np_preds, 'inphynet', os.path.join('checkpoints_inphynet_doc2vec', FLAGS.log_file), epoch)
	...
	# print(f'Epoch: {epoch}, Primary Loss: {prim_loss.item():.4f}')
	print(f'Epoch: {epoch}')
	...
	if(epoch%5==0):
		...
		torch.save(prim_state, os.path.join('checkpoints_inphynet_doc2vec', FLAGS.primary_task_network_save))

		aux_state={'epoch': epoch, 'state_dict': aux_task_network.state_dict(), 'optimizer': atn_optimizer.state_dict()}
		torch.save(aux_state, os.path.join('checkpoints_inphynet_doc2vec', FLAGS.aux_task_network_save))

		wal_state={'epoch': epoch, 'state_dict': weight_alignment_network.state_dict(), 'optimizer': wal_optimizer.state_dict()}
		torch.save(wal_state, os.path.join('checkpoints_inphynet_doc2vec', FLAGS.weight_alignment_layer_save))

		pickle.dump([primary_task_network.word_embeddings.weight, primary_task_network.word_embeddings.weight.shape], open(os.path.join('checkpoints_inphynet_doc2vec', FLAGS.prim_vocab_weights_save), 'wb'))
		pickle.dump([aux_task_network.word_embeddings.weight, aux_task_network.word_embeddings.weight.shape], open(os.path.join('checkpoints_inphynet_doc2vec', FLAGS.aux_vocab_weights_save), 'wb'))

pickle.dump(aux_loss_history, open('inphynet_aux_loss.pickle', 'wb'))
pickle.dump(prim_loss_history, open('inphynet_prim_loss.pickle', 'wb'))
pickle.dump(total_loss_history, open('inphynet_total_loss.pickle', 'wb'))
