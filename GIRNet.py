...
class InterleaveLSTMCells( keras.layers.Layer ) :
    ...
    def call(self, inputs, states):
        ...
        for  i in range(len(self.cells)):
            # _ , [ h_primaux[i] , c_primaux[i] ]  = self.cells[i]( inputs , states )
            _ , [ h_primaux[i] , c_primaux[i] ]  = self.cells[i].call( states=tf.convert_to_tensor(states), inputs=tf.convert_to_tensor(inputs) )
