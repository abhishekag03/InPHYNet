...
if not os.path.exists('results_vanilla_lstm_tfidf'):
		os.makedirs('results_vanilla_lstm_tfidf')
...
embeddings_dimension=pickle.load(open(os.path.join('checkpoints_vanilla_lstm_tfidf', FLAGS.prim_vocab_weights_save), 'rb'))
vocab_dims=embeddings_dimension[1][0]

primary_task_network=PrimaryTaskNetworkTFIDF(FLAGS.batch_size, FLAGS.prim_num_multi_classes, FLAGS.num_hidden, vocab_dims, FLAGS.embedding_length, word_embeddings)
...
for idx, batch in enumerate(test_iterator):
	try:
		...
		print_evaluation_metrics(np_targets, np_preds, 'PTN', os.path.join('results_vanilla_lstm_tfidf', FLAGS.log_file))
